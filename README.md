...
`npm test` runs repository validation, authorization-policy checks, and behavior-level tests for selected order and messaging Cloud Functions. The behavior suite executes the same dependency-injected handlers exported by the deployed entry points, with deterministic in-memory database adapters and no private cloud credentials. Tests cover atomic order creation, order/message request replay, message rate limiting, and transactional unread updates; real CloudBase transaction execution still requires isolated-environment verification.
